import { Client, GatewayIntentBits, Partials, ActivityType, Routes, SlashCommandBuilder, EmbedBuilder, PermissionFlagsBits } from 'discord.js';
...
const COMMANDS = [
  ...
  { name: 'userinfo', description: 'Muestra información detallada de un usuario.', option: 'user', category: 'moderation', permission: PermissionFlagsBits.KickMembers },
...
function commandDefinitions(settings = {}) {
  ...
  return COMMANDS.filter(command => enabled[command.name] !== false).map(command => {
    ...
    if (command.option === 'user' || command.option === 'user_reason' || command.option === 'user_reason_required') {
      builder.addUserOption(option => option.setName('usuario').setDescription('Usuario a consultar.').setRequired(true));
    }
    if (command.option === 'user_reason' || command.option === 'user_reason_required') {
      builder.addStringOption(option => option.setName('razon').setDescription('Razón de la moderación.').setMaxLength(500).setRequired(command.option === 'user_reason_required'));
    }
...
export class MultibotManager {
  ...
  async registerInstanceCommands(instance, botRecord) {
    if (!instance?.application?.commands) return false;
    const id = Number(botRecord.id); const commands = commandDefinitions(botRecord.settings || {}).map(command => command.toJSON());
    try { await instance.application.commands.set(commands); this.commandRegistration.set(id, commands.map(command => command.name)); logger.info(`[multibot] Registered ${commands.length} commands for instance ${id}`); return true; }
    catch (error) { logger.error(`[multibot] Failed to register commands for instance ${id}: ${error?.message || error}`); return false; }
  }
  ...
  async handleCommand(interaction, botRecord) {
    ...
    if (command === 'server') {
      ...
      const guild = interaction.guild; return interaction.reply({ embeds: [new EmbedBuilder().setTitle(`🏠 ${guild.name}`).addFields({ name: 'Miembros', value: String(guild.memberCount), inline: true }, { name: 'Canales', value: String(guild.channels.cache.size), inline: true }).setColor(0x7c5cff)] });
    }
    if (command === 'user' || command === 'avatar') {
      const user = interaction.options.getUser('usuario') || interaction.user;
      if (command === 'user') return interaction.reply({ content: `👤 **${user.tag}**\nID: \`${user.id}\`\nCuenta creada: <t:${Math.floor(user.createdTimestamp / 1000)}:F>`, ephemeral: true });
      return interaction.reply({ content: `🖼️ Avatar de **${user.tag}**\n${user.displayAvatarURL({ size: 1024, extension: 'png' })}` });
    }
    if (command === 'userinfo') {
      const user = interaction.options.getUser('usuario'); const member = interaction.guild?.members.cache.get(user.id) || await interaction.guild?.members.fetch(user.id).catch(() => null);
      if (!member) return interaction.reply({ content: '❌ No pude encontrar a ese miembro en este servidor.', ephemeral: true });
      const roles = member.roles.cache.filter(role => role.id !== interaction.guild.id).map(role => role.toString()).slice(0, 15).join(' ') || 'Sin roles';
      return interaction.reply({ embeds: [new EmbedBuilder().setTitle(`👤 ${user.tag}`).setThumbnail(user.displayAvatarURL({ size: 256 })).addFields({ name: 'ID', value: user.id, inline: true }, { name: 'Cuenta', value: `<t:${Math.floor(user.createdTimestamp / 1000)}:F>`, inline: true }, { name: 'Entrada', value: member.joinedTimestamp ? `<t:${Math.floor(member.joinedTimestamp / 1000)}:F>` : 'Desconocida', inline: true }, { name: 'Roles', value: roles }).setColor(0x7c5cff)] });
    }
    if (!interaction.guild) return interaction.reply({ content: '❌ Este comando solo funciona dentro de un servidor.', ephemeral: true });
    if (command === 'ban' || command === 'kick') {
      const target = interaction.options.getUser('usuario'); const reason = formatReason(interaction.options.getString('razon'));
      if (targetIsSelf(interaction, target)) return interaction.reply({ content: '❌ No puedes moderarte a ti mismo.', ephemeral: true });
      if (targetIsBot(interaction, target)) return interaction.reply({ content: '❌ No puedes moderar a este bot.', ephemeral: true });
      const member = await interaction.guild.members.fetch(target.id).catch(() => null);
      if (member && !member.moderatable) return interaction.reply({ content: '❌ No puedo moderar a ese usuario. Revisa mi rol y la jerarquía de roles.', ephemeral: true });
      if (command === 'ban') { await interaction.guild.members.ban(target.id, { reason }); return interaction.reply({ content: `🔨 **${target.tag}** fue baneado.\nRazón: ${reason}` }); }
      if (!member) return interaction.reply({ content: '❌ Ese usuario no está en el servidor.', ephemeral: true });
      await member.kick(reason); return interaction.reply({ content: `👢 **${target.tag}** fue expulsado.\nRazón: ${reason}` });
    }
    if (command === 'timeout') {
      const target = interaction.options.getUser('usuario'); const minutes = interaction.options.getInteger('minutos'); const reason = formatReason(interaction.options.getString('razon')); const member = await interaction.guild.members.fetch(target.id).catch(() => null);
      if (!member) return interaction.reply({ content: '❌ Ese usuario no está en el servidor.', ephemeral: true });
      if (targetIsSelf(interaction, target) || targetIsBot(interaction, target)) return interaction.reply({ content: '❌ No puedes aplicar timeout a ese usuario.', ephemeral: true });
      if (!member.moderatable) return interaction.reply({ content: '❌ No puedo aplicar timeout. Revisa la jerarquía de roles.', ephemeral: true });
      await member.timeout(minutes * 60 * 1000, reason); return interaction.reply({ content: `⏱️ **${target.tag}** recibió un timeout de **${minutes} minuto(s)**.\nRazón: ${reason}` });
    }
    if (command === 'clear') {
      const amount = interaction.options.getInteger('cantidad');
      if (!interaction.channel?.isTextBased?.() || typeof interaction.channel.bulkDelete !== 'function') return interaction.reply({ content: '❌ Este comando no funciona en este canal.', ephemeral: true });
      const deleted = await interaction.channel.bulkDelete(amount, true); return interaction.reply({ content: `🧹 Eliminé **${deleted.size}** mensaje(s).`, ephemeral: true });
    }
    if (command === 'lock' || command === 'unlock') {
      if (!interaction.channel?.permissionOverwrites) return interaction.reply({ content: '❌ No puedo modificar los permisos de este canal.', ephemeral: true });
      await interaction.channel.permissionOverwrites.edit(interaction.guild.roles.everyone, { SendMessages: command === 'unlock' ? null : false }, { reason: `${command === 'lock' ? 'Bloqueo' : 'Desbloqueo'} ejecutado por ${interaction.user.tag}` });
      return interaction.reply({ content: command === 'lock' ? '🔒 Canal bloqueado para @everyone.' : '🔓 Canal desbloqueado para @everyone.' });
    }
    if (command === 'warn') {
      const target = interaction.options.getUser('usuario'); const reason = formatReason(interaction.options.getString('razon'));
      if (targetIsSelf(interaction, target) || targetIsBot(interaction, target)) return interaction.reply({ content: '❌ No puedes advertir a ese usuario.', ephemeral: true });
      const member = await interaction.guild.members.fetch(target.id).catch(() => null); if (!member) return interaction.reply({ content: '❌ Ese usuario no está en el servidor.', ephemeral: true });
      const warning = await addWarning(this.ownerClient.db, { botInstanceId: botRecord.id, guildId: interaction.guild.id, userId: target.id, moderatorId: interaction.user.id, reason });
      return interaction.reply({ content: `⚠️ **${target.tag}** recibió una advertencia #${warning.id}.\nRazón: ${reason}` });
    }
    if (command === 'warnings') {
      const target = interaction.options.getUser('usuario'); const warnings = await listWarnings(this.ownerClient.db, { botInstanceId: botRecord.id, guildId: interaction.guild.id, userId: target.id, limit: 10 });
      if (!warnings.length) return interaction.reply({ content: `📋 **${target.tag}** no tiene advertencias registradas.`, ephemeral: true });
      const description = warnings.map((warning, index) => `**#${warning.id}** · <t:${Math.floor(new Date(warning.created_at).getTime() / 1000)}:d>\n${formatReason(warning.reason)} · Moderador: <@${warning.moderator_id}>`).join('\n\n');
