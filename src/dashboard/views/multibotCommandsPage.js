...
export function renderMultibotCommands({ user, bot, csrf }) {
  const settings = bot?.settings && typeof bot.settings === 'object' ? bot.settings : {};
  ...
  const voiceEnabled = enabled.voice !== false;
  ...
  <div class="grid" style="max-width:900px">
    <div class="card">
      ...
      <div class="row" style="margin-top:18px;flex-wrap:wrap"><button onclick="saveCommands()">${icon('check', 15)} Guardar comandos</button><a class="btn btn-ghost" href="/bots/${esc(botId)}">⚙️ Configuración</a><a class="btn btn-ghost" href="/bots">← Mis bots</a></div>
      <p id="msg" class="hint" style="min-height:20px;margin:12px 0 0"></p>
    </div>
    <div class="card"><h2>💡 Cómo funciona</h2><p class="hint">Los comandos se registran directamente en la aplicación de Discord de <strong>${esc(displayName)}</strong>. Al desactivar uno, desaparece de los comandos disponibles de ese bot.</p><p class="hint">Cada instancia mantiene su propia configuración. Los comandos de Wolf-Bot y de tus otras instancias no se modifican.</p><p class="hint">🔊 <strong>/voice</strong> usa una conexión de voz aislada por bot y servidor. <code>/voice join</code> te deja elegir el canal; <code>/voice leave</code> lo desconecta.</p><p class="hint">🛡️ <strong>Seguridad:</strong> los comandos de moderación siguen exigiendo los permisos correspondientes de Discord, y el comando de voz comprueba que el bot pueda ver y conectarse al canal seleccionado.</p></div>
  </div>
  <style>
  ...
  <script>
    const CSRF = ${JSON.stringify(csrf)}; const BOT_ID = ${JSON.stringify(botId)}; const VOICE_ENABLED = ${JSON.stringify(voiceEnabled)};
