...
export default {
    ...
    async execute(interaction, config) {
        ...
        try {
            ...
            if (options.length < 2) {
                throw new Error(t(lang, 'wolf.cmd.tools.poll.errMinOptions'));
            }
            ...
            if (isAnonymous) {
                description += t(lang, 'wolf.cmd.tools.poll.descAnonymous');
            } else {
                description += t(lang, 'wolf.cmd.tools.poll.descReact');
            }
            
            const embed = successEmbed(
                t(lang, isAnonymous ? 'wolf.cmd.tools.poll.titleAnonymous' : 'wolf.cmd.tools.poll.title'),
                description
            );
            ...
            await InteractionHelper.safeEditReply(interaction, {
                content: t(lang, 'wolf.cmd.tools.poll.success'),
            });
        } catch (error) {
