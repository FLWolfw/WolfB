...
export function renderMultibot({ user, bots, csrf }) {
  ...
  <script>
    ...
    async function startBot(id) {
      const msg = document.getElementById('msg-'+id); msg.textContent = 'Conectando con Discord…';
      try { await api('/api/multibot/'+id+'/start','POST'); location.reload(); }
      catch (e) { msg.textContent = 'No se pudo encender: ' + e.message; }
    }
